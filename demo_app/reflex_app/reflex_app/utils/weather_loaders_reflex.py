"""
Reflex port of demo_app/weather_phase2_loaders.py.

All @st.cache_* decorators removed. Cross-imports rewritten to use
sim_cache_reflex instead of sim_cache.
"""
...
from . import sim_cache_reflex as sim_cache  # noqa: E402
from .sim_cache_reflex import data_file_exists, repo_data_path  # noqa: E402

# RSC-SIM imports — sys.path already set up by sim_cache_reflex at import time
...
STARLINK_POL_LOSS_DB = 3.0

NEGLIGIBLE_RFI_DBW = -500.0
_RFI_ZERO_POWER_W = 1e-50
...
def _ws_ecef_at_time(ws_ecef_pos: pd.DataFrame, obs_time: datetime) -> np.ndarray:
...
def compute_rfi_time_series(
    ...
    Returns dict with minute axis, dBW arrays (as np.ndarray), n_emitters, obs_times iso list.
    Caller must call _to_serializable() before assigning to Reflex State.
    """
    ...
    if constellation is None:
        ...
        k_sl = _run_phase2_starlink(
            FREQ_K_HZ, k_inst, constellation, obs_times, sl_freq_hz, starlink_eirp_dbw,
        )
        v_sl = _run_phase2_starlink(
            FREQ_V_HZ, v_inst, constellation, obs_times, sl_freq_hz, starlink_eirp_dbw,
        )

    k_5g = _equivalent_5g_rfi_dbw_series(
        FREQ_K_HZ, k_inst, obs_times, emitter_density, five_g_eirp_dbw, fg_hz,
    )
    v_5g = _equivalent_5g_rfi_dbw_series(
        FREQ_V_HZ, v_inst, obs_times, emitter_density, five_g_eirp_dbw, fg_hz,
    )
