...
if direct_mode:
    if target_satellite:
        print(f"🎯 DIRECT EFFECTS MODE: Analyzing satellite '{target_satellite}'")
    else:
        print("🎯 DIRECT EFFECTS MODE: Will auto-select first visible satellite")
else:
    print("🌐 AGGREGATE EFFECTS MODE: Analyzing all visible satellites")
print("=" * 80)
...
if contamination_probability > 0.4:  # Medium or High risk
    ...
    if direct_mode and target_satellite is None:
        ...
        if len(visible_satellites) > 0:
            ...
            print(f"🎯 AUTO-SELECTED: First visible satellite '{target_satellite}' for direct effects analysis")
        else:
            print("⚠️ WARNING: No visible satellites found in observation window!")
...
if use_doppler_correction:
    ...
    if use_enhanced_transmitters:
        ...
        # Create constellation with the new link budget, direct effect, Use dynamic filtering based on analysis mode
        filt_funcs = get_filter_functions()
        starlink_constellation = Constellation.from_file(
            ...
            azimuth_tag='azimuths',
            distance_tag='ranges_westford',
            filt_funcs=filt_funcs  # direct effect: Dynamic filtering
        )
...
if direct_mode:
    print(f"   🎯 DIRECT EFFECTS: Analyzing {len(list_sats)} satellite(s)")
    for sat in list_sats:
        print(f"      • {sat}")
else:
    print(f"   🌐 AGGREGATE EFFECTS: Analyzing {len(list_sats)} satellites")
    print("   📋 ALL VISIBLE SATELLITES:")
    print(f"      {', '.join(list_sats)}")
    print(f"   📊 Total: {len(list_sats)} satellites")
...
if use_doppler_correction:
    ...
    if use_enhanced_transmitters:
        ...
        # direct effect: Use dynamic filtering based on analysis mode
        filt_funcs = get_filter_functions()
        starlink_constellation_freqs = Constellation.from_file(
