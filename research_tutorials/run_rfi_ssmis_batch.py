...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Parallel batch: run SSMI-S_RFI_modeling.py on every input .nc4 (5G + Starlink gateway RFI; "
            "e.g. util/SSMI-S). Skips ``*_RFI.nc4`` outputs from prior runs. Only SAID 285 (DMSP-F17) is "
            "processed. Optional script flags are not passed."
        )
    )
