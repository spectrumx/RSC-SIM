...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Parallel batch: run ATMS_RFI_modeling.py on every input .nc4 in a sensor directory "
            "(5G + Starlink gateway RFI; e.g. util/ATMS). Skips ``*_RFI.nc4`` outputs from "
            "prior runs. Optional script flags are not passed—use defaults (gateways CSV, in-FOV-first, etc.)."
        )
    )
    ...
    script_path = script_dir / "ATMS_RFI_modeling.py"
