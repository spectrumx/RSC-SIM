...
European Union's Global Human Settlement Layer (GHSL) population data (GHS-POP raster) needs to be downloaded from EU [Global Human Settlement Layer (GHSL)](https://human-settlement.emergency.copernicus.eu/download.php?ds=pop) for Epoch: 2025, Resolution: 30 arcsec (~1 km²), and Coordinate system: WGS84. It is zip compressed but only `GHS_POP_E2025_GLOBE_R2023A_4326_30ss_V1_0.tif` file is required. Please place it at `research_tutorials/data/`. It is used for 5G ground emitter density in ATMS/AMSU-A/SSMI-S RFI scripts. Emitter density is assigned per km² from GHSL population and country allowlist (`country_5G_sensor_channel.csv`): **population > 5000 → 15 emitters/km²** (`GHSL_METRO_POP_THRESHOLD` in `src/weather_sat_nwp.py`); lower density → **0**. See `_population_to_density()` and `Input_parameters_ATMS_AMSU_A_SSMI-S_RFI_modeling.md` §5.1.

### 1-a. Metro-filtered GHSL (required for default RFI runs)

Default RFI scripts use **contiguous dense-urban metropolitan** cells only (not isolated dense speckles). Obtain the pre-built metro GeoTIFF from the project maintainer and place it alongside the native GHS-POP file:

`research_tutorials/data/GHS_POP_E2025_GLOBE_R2023A_4326_30ss_V1_0_metro.tif`

The file applies population **> 5000** per ~1 km² cell (`GHSL_METRO_POP_THRESHOLD`) and a **25 km²** minimum connected patch area. An optional JSON sidecar (`*_metro.json`) may accompany the share; RFI reads the `.tif` only. On HPC, copy the file to the same layout or set **`GHSL_METRO_TIF_PATH`**. **Legacy per-cell behavior** (original GeoTIFF, no contiguity filter): add **`--legacy-per-cell-5g`** to the RFI script command.
