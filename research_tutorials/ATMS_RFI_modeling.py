"""
RFI modeling for ATMS for NWP data biasing.

Computes 5G ground-emitter RFI (no Starlink) for all FOVs in ATMS netCDF-4 files.
Each nc4 contains data from multiple satellites (SUOMI-NPP, JPSS-1); SAID identifies
the satellite per observation. ECEF lookups are loaded per satellite from the
sensor directory. Runs a parametric study over ATMS channels 3–9.
...
Usage:
  python ATMS_RFI_modeling.py --sensor ATMS --nc4 path [--out_dir dir]
  e.g.:
...
def run_rfi_for_channel(
    ...
    """Run RFI model for one ATMS channel; write CSV with timestamp, satellite, lat, lon, saza, rfi_dBW, rfi_Tb."""
    lat = data["lat"]
    lon = data["lon"]
    saza = data["saza"]
    ...
    # Single-pass grouping: (ts, sat) -> indices + coords; no set(zip(...)), no mask builds, no repeat lookups
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ATMS RFI modeling for NWP (5G ground emitters only; multi-satellite per nc4)."
    )
    parser.add_argument(
        "--sensor",
        required=True,
    ...
    parser.add_argument(
        "--nc4",
        required=True,
        help="Path to ATMS netCDF-4 file (e.g. util/ATMS/atms_2023080112.nc4). ECEF lookups loaded from same dir.",
    )
    parser.add_argument(
        "--out_dir",
        default=None,
