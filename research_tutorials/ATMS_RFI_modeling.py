"""
RFI modeling for ATMS/JPSS-1 (NOAA-20) for NWP data biasing.

Computes 5G ground-emitter RFI (no Starlink) for all FOVs in ATMS netCDF-4 files,
using ECEF lookup for satellite position per timestamp. Runs a parametric study
over ATMS channels 3–9: one output CSV per channel, with emitter fundamental set
...
Usage:
  python ATMS_RFI_modeling.py --sat SATELLITE_NAME [--nc4 path] [--ecef path] [--out_dir dir]
  e.g.:
  python ATMS_RFI_modeling.py --sat JPSS-1 --nc4 util/JPSS-1/atms_2023080112.nc4 --ecef util/JPSS-1/JPSS-1_ECEF_lookup_atms_2023080112.csv --out_dir util/JPSS-1
  Output: <out_dir>/<satellite>_<nc4_basename>_5G_RFI_chN.csv (e.g. JPSS-1_atms_2023080112_5G_RFI_ch3.csv).
...
# V-band (50–55.5 GHz), ATMS Ch 3–9: parametric study over channels
...
# nc4 variable names (Satellite_Info_DK1 ATMS CSV.csv)
VAR_LAT = "LAT"
VAR_LON = "LON"
VAR_SAZA = "SAZA"
VAR_BEARAZ = "BEARAZ"
...
def run_rfi_for_channel(
    ...
    """Run RFI model for one ATMS channel; write CSV. emitter_fundamental_hz sets 2nd harmonic (e.g. center_freq/2)."""
    lat = data["lat"]
    lon = data["lon"]
    saza = data["saza"]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ATMS/JPSS-1 RFI modeling for NWP (5G ground emitters only)."
    )
    parser.add_argument(
        "--sat",
        required=True,
        metavar="SATELLITE",
        help="Satellite name for output filenames (e.g. JPSS-1, SUOMI-NPP).",
    )
    parser.add_argument(
        "--nc4",
        required=True,
        help="Path to ATMS netCDF-4 file (default: data/atms_2023080112.nc4)",
    )
    parser.add_argument(
        "--ecef",
        required=True,
        help="Path to JPSS-1 ECEF lookup CSV",
    )
    parser.add_argument(
        "--out_dir",
        required=True,
